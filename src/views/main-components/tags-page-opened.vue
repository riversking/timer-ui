...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      currentClosePage() {
        return this.$store.state.global.currentClosePage
      }
    },
    methods: {
      itemTitle (item) {
      ...
      closePage (event, name) {
        ...
        if (this.currentPageName === name) {
          ...
          let backToList = null
          if (name.lastIndexOf('Content') !== -1) {
            backToList = name.replace('Content', '')
          }
          if (backToList !== null) {
            for (let i = 1; i < len; i++) {
              if (pageOpenedList[i].name === backToList) {
    ...
    watch: {
      ...
      currentClosePage (val, oldVal) {
        // console.log(val, oldVal)
        this.closePage(null, val[0])
      }
    }
  };
</script>
